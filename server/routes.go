...
func Routes(f *fiber.App, db *gorm.DB) {
	...
	customerItemPriceRepo := repository.NewCustomerItemPriceRepository(db)
	customerItemPriceUsecase := usecase.NewCustomerItemPriceUsecase(txManager, customerItemPriceRepo, customerRepo, masterItemRepo, auditLogRepo)
	...
	customerItemPriceHandler := handler.NewCustomerItemPriceHandler(customerItemPriceUsecase)
	...
	customerPricingRead := protected.Group("", middleware.Authorize(rbacRepo, constant.PermCustomerPricingRead))
	customerPricingRead.Get("/customers/:customerId/pricing", customerItemPriceHandler.FindAllByCustomer)
	customerPricingRead.Get("/customers/:customerId/items/:masterItemId/pricing", customerItemPriceHandler.FindActive)

	customerPricingWrite := protected.Group("", middleware.Authorize(rbacRepo, constant.PermCustomerPricingWrite))
	customerPricingWrite.Post("/customers/:customerId/pricing", customerItemPriceHandler.Create)
	customerPricingWrite.Put("/customers/:customerId/pricing/:id", customerItemPriceHandler.Update)
	customerPricingWrite.Delete("/customers/:customerId/pricing/:id", customerItemPriceHandler.Delete)

	customerPORead := protected.Group("", middleware.Authorize(rbacRepo, constant.PermCustomerPORead))
	customerPORead.Get("/sales/customer-pos", customerPOHandler.FindAll)
	customerPORead.Get("/sales/customer-pos/:id", customerPOHandler.FindById)

	customerPOWrite := protected.Group("", middleware.Authorize(rbacRepo, constant.PermCustomerPOWrite))
	customerPOWrite.Post("/sales/customer-pos", customerPOHandler.Create)
	customerPOWrite.Put("/sales/customer-pos/:id/confirm", customerPOHandler.Confirm)

	salesOrderRead := protected.Group("", middleware.Authorize(rbacRepo, constant.PermSalesOrderRead))
	salesOrderRead.Get("/sales/sales-orders", salesOrderHandler.FindAll)
	salesOrderRead.Get("/sales/sales-orders/:id", salesOrderHandler.FindById)

	salesOrderWrite := protected.Group("", middleware.Authorize(rbacRepo, constant.PermSalesOrderWrite))
	salesOrderWrite.Post("/sales/sales-orders", salesOrderHandler.Create)
	salesOrderWrite.Put("/sales/sales-orders/:id/confirm", salesOrderHandler.Confirm)
	salesOrderWrite.Put("/sales/sales-orders/:id/cancel", salesOrderHandler.Cancel)
	...
	driverRead := protected.Group("", middleware.Authorize(rbacRepo, constant.PermDriverRead))
	driverRead.Get("/logistics/drivers", driverHandler.FindAll)
	driverRead.Get("/logistics/drivers/:id", driverHandler.FindById)

	driverWrite := protected.Group("", middleware.Authorize(rbacRepo, constant.PermDriverWrite))
	driverWrite.Post("/logistics/drivers", driverHandler.Create)
	driverWrite.Put("/logistics/drivers/:id", driverHandler.Update)
	driverWrite.Delete("/logistics/drivers/:id", driverHandler.Delete)
